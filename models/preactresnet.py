...
class PreActResNet(nn.Module):
    ...
    def __init__(self, block, num_block, class_num=100, channel_size=3):
        super().__init__()
        self.input_channels = 64

        self.pre = nn.Sequential(
            nn.Conv2d(channel_size, 64, 3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        )
...
def preactresnet18(args):
    return PreActResNet(PreActBasic, [2, 2, 2, 2], class_num=args.nc, channel_size=args.cs)
    
def preactresnet34(args):
    return PreActResNet(PreActBasic, [3, 4, 6, 3], class_num=args.nc, channel_size=args.cs)

def preactresnet50(args):
    return PreActResNet(PreActBottleNeck, [3, 4, 6, 3], class_num=args.nc, channel_size=args.cs)

def preactresnet101(args):
    return PreActResNet(PreActBottleNeck, [3, 4, 23, 3], class_num=args.nc, channel_size=args.cs)

def preactresnet152(args):
    return PreActResNet(PreActBottleNeck, [3, 8, 36, 3], class_num=args.nc, channel_size=args.cs)
