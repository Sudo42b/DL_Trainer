...
class GoogleNet(nn.Module):
    ...
    def __init__(self, num_class=100, channel_size=3):
        super().__init__()
        self.prelayer = nn.Sequential(
            nn.Conv2d(channel_size, 192, kernel_size=3, padding=1),
            nn.BatchNorm2d(192),
            nn.ReLU(inplace=True)
        )
...
def googlenet(args):
    return GoogleNet(num_class=args.nc, channel_size = args.cs)
