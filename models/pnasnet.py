...
class PNASNet5Large(nn.Module):
    def __init__(self, num_classes=1001, channels=3):
        super(PNASNet5Large, self).__init__()
        self.num_classes = num_classes
        self.conv_0 = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(channels, 96, kernel_size=3, stride=2, bias=False)),
...
def pnasnet(args, pretrained=False):
    r"""PNASNet-5 model architecture from the
    `"Progressive Neural Architecture Search"
    <https://arxiv.org/abs/1712.00559>`_ paper.
    """
    if pretrained:
        ...
        model = PNASNet5Large(num_classes=args.nc, channels=args.cs)
    return model