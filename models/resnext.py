...
class ResNext(nn.Module):
    ...
    def __init__(self, block, num_blocks, class_num=100, channel_size=3):
        super().__init__()
        self.in_channels = 64

        self.conv1 = nn.Sequential(
            nn.Conv2d(channel_size, 64, 3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        )
...
def resnext50(args):
    ...
    """
    return ResNext(ResNextBottleNeckC, [3, 4, 6, 3], class_num=args.nc, channel_size=args.cs)

def resnext101(args):
    """ return a resnext101(c32x4d) network
    """
    return ResNext(ResNextBottleNeckC, [3, 4, 23, 3], class_num=args.nc, channel_size=args.cs)

def resnext152(args):
    """ return a resnext101(c32x4d) network
    """
    return ResNext(ResNextBottleNeckC, [3, 4, 36, 3], class_num=args.nc, channel_size=args.cs)
