...
class NASNetAMobile(nn.Module):
    ...
    def __init__(self, num_classes=1000, stem_filters=32, penultimate_filters=1056, filters_multiplier=2, channels=3):
        ...
        self.conv0.add_module('conv', nn.Conv2d(in_channels=channels, out_channels=self.stem_filters, kernel_size=3, padding=0, stride=2,
                                                bias=False))
...
def nasnet_Mobile(args):
    r"""NASNetALarge model architecture from the
    `"NASNet" <https://arxiv.org/abs/1707.07012>`_ paper.
    """
    model = NASNetAMobile(num_classes=args.nc, channels=args.cs)
    
    return model

if __name__ == "__main__":

    model = NASNetAMobile(channels=1, num_classes=8)
    input = Variable(torch.randn(2, 1, 32*7, 32*7))
    output = model(input)
